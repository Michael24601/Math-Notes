...
\[ \int_\dcal f(x) dx
= v_d(\dcal)\mu(f(x)) \]
Where we can estimate the average by just taking
$N$ random samples and dividing by $N$:
\[ \int_\dcal f(x) dx
\approx \dfrac{v_d(\dcal)}{N} \sum_{i=1}^N f(x_i) \]
This is how Monte-Carlo integration works. \\'

This is acyually a special case of Monte-Carlo
integration where the points we sample are sampled
uniformally (randomly). \\
In general, we have:
\[ \int_\dcal f(x) dx
\approx \dfrac{1}{N} \sum_{i=1}^N \dfrac{f(x_i)}{p(x_i)} \]
where $p(x)$ is the probability desnity function of
picking a point $x$. \\
This function happens to be the reciprocal of 
the volume if the distribution is uniform. \\ 

One trick we can use
...
Another note is that spherical coordinates
transformation $T\inv$ doesn't
actually map 3 dimensional points uniformally
on a sphere (or hemisphere). So we need
to divide by a probability desnity function. \\
However, the change of measure term, which is
the determinant of the Jacobian of $T$,
actually gives us the probability density function. \\
This is because the determinant of the Jacobian
of the inverse transform scales the probability
density function:
\[ |\det(J_T)| = \dfrac{1}{|\det(J_{T})\inv|}
= \dfrac{1}{|\det(J_{T\inv})|} \]
So this term will scale the probability
density function of the original samples $u$, and $v$
to become correct given the new transform.
And since they were uniformally distributed
in a unit hypercube, the change of measure term
is our probability density function reciprocal. \\
...
we can use the density function to determine which
surface the number points to. \\
If we don't use the density function in order to
ensure uniform sampling. Otherwise, we would have
to divide by the probability density function
that comes from picking surfaces without taking
surface area into consideration. \\
